...
{
    return this->channel_name; // ADDED "/" HERE
}
...
void Event:: split_str(const std::string& line, char key, std::vector<std::string>& output)
{
    std::stringstream ss(line); // Create a stringstream from the input string
    std::string item;
    while (std::getline(ss, item, key)) { // Extract substrings separated by the delimiter
        output.push_back(item); // Add each substring to the output vector
    }
}
...
Event::Event(const std::string &frame_body, const std::string & channelName): channel_name(channelName), city(""), 
                                             name(""), date_time(0), description(""), general_information(),
                                             eventOwnerUser("")
{
    stringstream ss(frame_body);
    ...
    while(getline(ss,line,'\n')){
        ...
        if(line.find(':') != string::npos) {
            ...
            if(inGeneralInformation) {
                general_information_from_string[key] = val; // general_information_from_string[key.substr(1)] = val;
            }
        }
    }
    general_information = general_information_from_string;
